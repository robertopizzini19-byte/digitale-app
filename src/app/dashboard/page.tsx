...
function BrandLogo({ size = "default", variant = "dark" }: { size?: "sm" | "default" | "lg"; variant?: "dark" | "light" }) {
  const cfg = {
    ...
    default: { dig: "text-[1.2rem]", ita: "text-[1.55rem]", le: "text-[1.05rem]", bar: "h-[2px]" },
    lg:      { dig: "text-[1.35rem]", ita: "text-[1.75rem]", le: "text-[1.2rem]",  bar: "h-[2px]" },
  };
  const s = cfg[size];
  ...
  return (
    <span className="inline-flex flex-col select-none" aria-label="DigITAle">
      ...
      <span className={`brand-underline ${s.bar} mt-[2px] rounded-full`} style={{ width: "100%" }} />
    </span>
  );
}
